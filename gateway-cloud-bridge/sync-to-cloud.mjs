...
async function tick(){
  ...
  console.log(new Date().toLocaleTimeString(),`P4A sync OK | matches=${matches.length} prelive=${prelive} live=${live} signals=${signals.length} resolved=${resolvedSignals} events=${events.length} tickets=${tickets.filter(x=>x.status==="READY").length}/4${baselineOnly?" | BASELINE BLOQUEADA":""}`);
...
console.log(`MatchIntel Cloud Bridge v${VERSION} ativo | P3 TELEGRAM E2E + P4A DAILY TICKETS | intervalo ${every/1000}s`);
console.log("P4A publica bilhetes do Gateway em canal cloud separado; odds exibidas pelo módulo são odds justas do modelo, nunca odds inventadas de bookmaker.");
