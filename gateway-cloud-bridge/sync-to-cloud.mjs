...
async function tick(){
  ...
  console.log(new Date().toLocaleTimeString(),`P11.0.2 sync OK | matches=${matches.length} prelive=${prelive} live=${live} signals=${signals.length} resolved=${resolvedSignals} events=${events.length} tickets=${readyTickets}/4 values=${valueReady}+${valueStrong}F${perfLabel}${btLabel}${histLabel}${baselineOnly?" | BASELINE BLOQUEADA":""}`);
...
console.log(`MatchIntel Cloud Bridge v${VERSION} ativo | P3 TELEGRAM + P4A TICKETS + P4C2 VALUE + P5 PERFORMANCE + P6 BACKTEST + P7 HISTORY + P11.0.2 FRESHNESS | intervalo ${every/1000}s`);
console.log("P11.0.2 usa providerFetchedAt para LIVE; bridgeSyncedAt fica separado. P7/P6 continuam preservados.");
