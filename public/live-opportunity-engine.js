...
/* P11_0_5_2_LIVE_FRESHNESS_DEFENSE */
function oeProviderTime(m){const mi=m?.stats?._matchintel||{},sm=m?.source_matrix||{},raw=[m?.provider_fetched_at,mi.providerFetchedAt,sm.providerFetchedAt,sm.provider_fetched_at,...(Array.isArray(sm.observations)?sm.observations.flatMap(o=>[o?.fetchedAt,o?.fetched_at,o?.observedAt,o?.timestamp]):[])];const xs=raw.map(v=>new Date(typeof v==='number'&&v<1e12?v*1000:v||0).getTime()).filter(Number.isFinite);return xs.length?new Date(Math.max(...xs)).toISOString():null}
function oeBridgeTime(m){return m?.stats?._matchintel?.bridgeSyncedAt||m?.updated_at||null}
function oeProviderAuthority(m){const basis=String(m?.freshness_basis||m?.stats?._matchintel?.freshnessBasis||'').toUpperCase();return basis==='PROVIDER'&&!!oeProviderTime(m)}
function oeFresh(m){if(oeLive(m)&&!oeProviderAuthority(m))return false;const a=oeAge(oeProviderTime(m));return a>=-120000&&a<=OE_FRESH_MS}
...
async function oeLoad(){const c=oeCfg(),root=document.querySelector('#liveOpportunityEngine');if(!root)return;if(!c){setTimeout(oeLoad,800);return}try{const [matches,signals]=await Promise.all([oeQ(c,'matchintel_match_lifecycle','select=*&lifecycle_state=eq.ATIVO&order=updated_at.desc&limit=500'),oeQ(c,'matchintel_signals','select=*&order=created_at.desc&limit=200')]);const live=matches.filter(m=>String(m?.lifecycle_state||'').toUpperCase()==='ATIVO'&&oeLive(m)&&oeRelayCurrent(m)),rows=live.map(m=>{const ss=oeSignals(m,signals),e=oeEvaluate(m,ss);return {m,ss,e,score:oeScore(m,e)}}).sort((a,b)=>b.score-a.score),sum=oeSummary(rows);const badge=document.querySelector('#liveOpportunityBadge');if(badge)badge.textContent=`${sum.ELITE} elite · ${sum.FORTE} forte · ${sum.OPORTUNIDADE} oportunidade`;if(!live.length){root.innerHTML=`<div class="oe-empty"><strong>Opportunity Engine armado · 0 jogos LIVE operacionais</strong><span>Quando surgir uma partida ao vivo, o P9.1 começa em AQUECENDO/EVITAR e só promove OPORTUNIDADE, FORTE ou ELITE se todos os gates correspondentes forem satisfeitos.</span></div>`;return}root.innerHTML=`<div class="oe-summary"><span><small>Aquecendo</small><b>${sum.AQUECENDO}</b></span><span><small>Oportunidade</small><b>${sum.OPORTUNIDADE}</b></span><span><small>Forte</small><b>${sum.FORTE}</b></span><span><small>Elite</small><b>${sum.ELITE}</b></span><span><small>Evitar</small><b>${sum.EVITAR}</b></span></div><div class="oe-list">${rows.map(oeCard).join('')}</div><p class="oe-note">P11.0.5.2 exige provider timestamp FRESH e freshnessBasis=PROVIDER para qualquer promoção LIVE. Lifecycle já exclui LIVE sem tempo autoritativo ou stale. P9.1 permanece SHADOW. “Oportunidade/Forte/Elite” é uma classificação operacional do MatchIntel, não execução automática. Odds não são inventadas; P11.0.2 exige timestamp esportivo FRESH para promoção. Relay recente não transforma snapshot antigo em dado fresco.</p>`;}catch(e){root.innerHTML=`<div class="oe-empty"><strong>Opportunity Engine indisponível</strong><span>${oeEsc(e.message)}</span></div>`}}
