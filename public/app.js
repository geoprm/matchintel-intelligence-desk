...
const empty=(title,body)=>`<div class="empty"><div class="icon">â—Œ</div><h3>${esc(title)}</h3><p>${esc(body)}</p></div>`;
const fmtTime=(d)=>{try{return new Date(d).toLocaleString("pt-BR",{hour:"2-digit",minute:"2-digit",day:"2-digit",month:"2-digit"})}catch{return "â€”"}};
const minuteLabel=(m,phase)=>m!=null?`${m}'`:(phase||"â€”");
...
const scheduledLabel=(m)=>{const d=scheduledAt(m);if(!d)return m.phase||m.state||"PrÃ©-live";try{return new Date(d).toLocaleString("pt-BR",{hour:"2-digit",minute:"2-digit"})}catch{return "PrÃ©-live"}};
...
function renderStatus(){
  ...
  $("#statusPills").innerHTML=[
    ...
    `<span class="pill ${telegramOn&&!stale?"ok":"warn"}">Telegram ${telegramOn&&!stale?"ON":"â€”"}</span>`,
    `<span class="pill ${s?.shadow_mode?"warn":"ok"}">${s?.shadow_mode?"SHADOW":"LIVE"}</span>`
  ].join("");
  if(!s?.last_sync_at) $("#lastSync").textContent="Sem sincronizaÃ§Ã£o";
  else if(stale) $("#lastSync").textContent=`Gateway sem sync Â· Ãºltima ${fmtTime(s.last_sync_at)}`;
  else $("#lastSync").textContent=`Sync ${fmtTime(s.last_sync_at)}`;
}
function matchCard(m){
  const score=(m.home_score!=null&&m.away_score!=null)?`${m.home_score}â€“${m.away_score}`:"â€”";
  const prob=m.best_probability!=null?Math.round(Number(m.best_probability)):"â€”";
  ...
  return `<div class="match" data-match="${esc(m.match_key)}">
    ...
    <div class="teams"><strong>${esc(m.home)} Ã— ${esc(m.away)}</strong><small>${esc(m.competition||m.radar_state||m.state||"")}</small></div>
    <div class="score">${score}</div>
    <div class="market">${esc(m.best_market||m.radar_state||"")}<b>${prob}${prob!=="â€”"?"%":""}</b></div>
  </div>`;
}
function renderHome(){
  ...
  $("#priorityList").innerHTML=priorities.length?priorities.map(signalCard).join(""):empty("Sem prioridade crÃ­tica","Nenhum PINNED/JOGO APITADO fresco recebido nos Ãºltimos 5 minutos.");
  ...
  if(preEl) preEl.innerHTML=prelive.length?prelive.map(matchCard).join(""):empty("Radar prÃ©-live aguardando partidas","O MatchIntel deve receber a agenda do Gateway mesmo sem sinais do Telegram.");
  ...
  $("#homeLive").innerHTML=live.length?live.map(matchCard).join(""):empty(
    gatewayFresh()?"Nenhum jogo ao vivo no relay":"Gateway sem atualizaÃ§Ã£o",
    gatewayFresh()?"O radar autÃ´nomo continuarÃ¡ procurando partidas independentemente do Telegram.":"Partidas antigas ficam bloqueadas atÃ© uma nova sincronizaÃ§Ã£o."
  );
  $("#liveCount").textContent=`${live.length} jogo${live.length===1?"":"s"}`;

  const operational=[...prelive,...live];
  const metrics=[
    ["PrÃ©-live", prelive.length],
    ...
    ["Instant", operational.filter(m=>/instant|next|prÃ³xim/i.test(`${m.best_market} ${m.radar_state}`)).length],
    ["HT / Goal HT", operational.filter(m=>/ht|1h/i.test(`${m.best_market} ${m.phase} ${m.radar_state}`)).length],
    ["2Âº Tempo", operational.filter(m=>/2h|2Âº|segundo/i.test(`${m.best_market} ${m.phase} ${m.radar_state}`)).length],
  ];
  $("#radarCards").innerHTML=metrics.map(([label,n])=>`<div class="card kpi"><div class="label">${esc(label)}</div><div class="value">${n}</div><div class="sub">sessÃµes elegÃ­veis</div></div>`).join("");
  $("#homeSignals").innerHTML=activeSignals.length?activeSignals.slice(0,5).map(signalCard).join(""):empty("Sem sinais ativos","Telegram Ã© evidÃªncia adicional; o radar esportivo continua funcionando sozinho.");
}
function filterMatch(m,f){
  ...
  if(f==="instant") return /instant|next|prÃ³xim/.test(t);
  if(f==="HT") return /ht|1h/.test(t);
  if(f==="2H") return /2h|2Âº|segundo/.test(t);
...
function renderLive(){
  ...
  $("#liveList").innerHTML=rows.length?rows.map(matchCard).join(""):empty(
    state.liveFilter==="prelive"?"Nenhuma partida prÃ©-live recebida":(gatewayFresh()?"Nenhum jogo neste filtro":"Gateway sem atualizaÃ§Ã£o"),
    state.liveFilter==="prelive"?"O Bridge P2 procura Match Sessions e agenda em mÃºltiplas rotas locais do Gateway.":(gatewayFresh()?"O filtro mostra somente partidas reais e frescas.":"Nenhuma partida antiga Ã© tratada como ao vivo.")
  );
}
function providerKind(s){const x=`${s.provider_family} ${s.provider_name}`.toLowerCase();return x.includes("betzord")?"betzord":x.includes("mÃ¡fia")||x.includes("mafia")?"mafia":"other"}
...
function signalCard(s){
  ...
  const badges=[
    ...
    /MATCHINTEL/i.test(`${s.signal_type} ${s.state}`)?`<span class="badge shadow">Shadow / em calibraÃ§Ã£o</span>`:""
  ].join("");
  return `<div class="signal">
    ...
    <div class="signal-main">
      ...
      <small>${esc(s.market||"")} Â· ${ageLabel}</small>
      ${bookUrl?`<div class="signal-actions"><a class="bookmaker-link" href="${esc(bookUrl)}" target="_blank" rel="noopener noreferrer">${bookLabel} â†—</a></div>`:""}
    </div>
  </div>`;
}
function renderSignals(){
  ...
  const s=state.status;
  const sources=[
    ["API-Football",s?.api_provider?.toLowerCase().includes("football")||s?.auto_scan_active,"fixtures, placar e estatÃ­sticas via Gateway"],
    ["Telegram",!!s?.telegram_connected&&!(!s?.last_sync_at||Date.now()-new Date(s.last_sync_at).getTime()>90000),"Chat MÃ¡fia / BetZord via conta local"],
    ["Source Matrix",(s?.independent_sources||0)>0,`${s?.independent_sources||0} fonte(s) independente(s) no status global`],
    ["Bet365",false,"link original do Chat MÃ¡fia / view manual"],
    ["SofaScore / Flashscore",false,"planejadas/manual quando nÃ£o houver adapter ativo"]
  ];
...
function renderHistory(){
  ...
  $("#historyList").innerHTML=rows.length?rows.map(matchCard).join(""):empty("HistÃ³rico vazio","As sessÃµes reais acompanhadas pelo Gateway serÃ£o registradas aqui.");
}
function render(){
...
function openMatch(key){
  ...
  const statPairs=[
    ["Corners",stats.corners??stats.corner??"â€”"],["Chutes",stats.shots??stats.total_shots??"â€”"],
    ["No alvo",stats.shots_on_goal??stats.sot??"â€”"],["Vermelhos",stats.red_cards??stats.redCards??"â€”"]
  ];
  $("#matchDetail").innerHTML=`
    <div class="card priority">
      <div class="teams"><strong style="font-size:17px">${esc(m.home)} Ã— ${esc(m.away)}</strong><small>${esc(m.competition||"")}</small></div>
      <div style="display:flex;justify-content:space-between;align-items:end;margin-top:12px">
        ...
        <div class="score">${m.home_score??"â€”"} â€“ ${m.away_score??"â€”"}</div>
      </div>
    </div>
    <div class="section-title"><h2>Melhor leitura</h2><span>Probabilidade â‰  oportunidade</span></div>
    <div class="card">
      <div style="display:flex;justify-content:space-between;gap:12px"><div><strong>${esc(m.best_market||"Sem mercado elegÃ­vel")}</strong><p class="note">${esc(m.best_explanation||"Sem explicaÃ§Ã£o recebida do motor.")}</p></div><div style="font-size:27px;font-weight:800">${prob==null?"â€”":prob+"%"}</div></div>
      <span class="pill ${/elite|forte/i.test(m.best_level||"")?"ok":"warn"}">${esc(m.best_level||"Neutro")}</span>
      ${state.status?.shadow_mode?`<span class="badge shadow">Shadow / em calibraÃ§Ã£o</span>`:""}
    </div>
    <div class="section-title"><h2>Qualidade dos dados</h2><span>nÃ£o Ã© chance de acerto</span></div>
    <div class="card"><div style="display:flex;justify-content:space-between"><strong>${m.data_quality||0}%</strong><span class="note">${m.independent_sources||0} fonte(s) Â· ${m.conflicts||0} conflito(s)</span></div><div class="progress" style="margin-top:9px"><i style="width:${Math.max(0,Math.min(100,m.data_quality||0))}%"></i></div><p class="note">${esc(m.source_matrix_state||"Source Matrix sem estado informado.")}</p></div>
    <div class="section-title"><h2>EstatÃ­sticas presentes</h2><span>somente campos recebidos</span></div>
    <div class="stats">${statPairs.map(([a,b])=>`<div class="stat"><b>${esc(b)}</b><small>${a}</small></div>`).join("")}</div>
    <div class="section-title"><h2>Por quÃª / Riscos</h2><span>guardrails</span></div>
    ...
    <div class="card timeline">${ev.length?ev.map(e=>`<div class="event"><b>${esc(e.event_type)}</b>${e.side?` Â· ${esc(e.side)}`:""}<small>${e.observed_minute!=null?`${e.observed_minute}' observado`:`Detectado em ${fmtTime(e.detected_at)}`}</small></div>`).join(""):`<p class="note">Sem eventos registrados.</p>`}</div>
    <div class="section-title"><h2>Sinais ligados</h2><span>${sig.length}</span></div>
    <div class="card">${sig.length?sig.map(signalCard).join(""):empty("Sem sinais ligados","Esta sessÃ£o pode ter sido descoberta pelo scanner autÃ´nomo.")}</div>`;
...
function renderEvidence(){
  ...
  $("#evidenceGallery").innerHTML=state.evidence.map((x,i)=>`<div class="thumb"><img src="${x.data}" alt="evidÃªncia ${i+1}"><button data-del-e="${i}">Ã—</button></div>`).join("");
...
function setAlertsStatus(text,active=false){
  ...
  const btn=$("#alertsBtn");
  if(btn){
    btn.textContent=active?"ðŸ”” Alertas crÃ­ticos ativos":"ðŸ”” Ativar alertas crÃ­ticos";
...
async function ensurePushSubscription(sendTest=false){
  if(!("serviceWorker" in navigator)||!("PushManager" in window)||!("Notification" in window)){
    setAlertsStatus("Este navegador nÃ£o oferece Web Push compatÃ­vel.");
    return false;
  }
  const permission=Notification.permission==="granted"?"granted":await Notification.requestPermission();
  if(permission!=="granted"){
    setAlertsStatus(permission==="denied"?"NotificaÃ§Ãµes bloqueadas no Android/Chrome.":"PermissÃ£o de notificaÃ§Ã£o nÃ£o concedida.");
    return false;
  }
  ...
  setAlertsStatus("Alertas ativos neste aparelho. Push crÃ­tico + som do Android.",true);
  await playAlertTone();
  if(sendTest){
    try{
      ...
      console.error(e);
      setAlertsStatus("Alertas ativos. O teste remoto falhou; sua inscriÃ§Ã£o continua ativa.",true);
    }
  }
  return true;
}
async function initPushUI(){
  ...
  btn.onclick=async()=>{
    ...
    try{
      ...
      console.error(e);
      setAlertsStatus("NÃ£o consegui criar/registrar a inscriÃ§Ã£o de alertas. Toque para tentar novamente.",false);
    }finally{btn.disabled=false}
  };
  if(!("serviceWorker" in navigator)||!("PushManager" in window)||!("Notification" in window)){
    btn.classList.add("hidden");
    setAlertsStatus("Web Push nÃ£o suportado neste navegador.");
    return;
  }
